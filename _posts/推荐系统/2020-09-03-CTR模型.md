...
复杂度的降低：
$$
\sum_{i=1}^n \sum_{j=i+1}^n <v_i, v_j>x_i x_j = \frac{1}{2}(\sum_{i=1}^n \sum_{j=1}^n <v_i, v_j>x_i x_j - \sum_{i=1}^n <v_i,v_i>x_i x_i)
$$
...
因此，化简后的公式为：
$$
\hat y (x) = w+0 + \sum_{i=1}^n w_i x_i + \frac{1}{2} \sum_{j=1}^k ((\sum_{i=1}^n v_{i,f} x_i)^2 - \sum_{i=1}^n v_{i,f}^2 x_i^2)
$$
其计算复杂度为O(kn)
...
$$
y(x) = w_0 + \sum_{i=1}^n w_i x_i +\sum_{i=1}^n \sum_{j=i+1}^n <v_{i,f_i}, v_{j,f_i}> x_i x_j
$$
