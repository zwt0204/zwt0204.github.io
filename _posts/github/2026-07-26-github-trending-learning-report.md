---
layout: post
title: "GitHub Trending 精读：ComposioHQ/awesome-claude-skills (2026-07-26)"
subtitle: "单个开源项目深度拆解"
date: 2026-07-26
author: "zwt"
...
- 语言：Python
- Stars：70,679，Forks：7,947，今日新增：577
- Topics：agent-skills、ai-agents、antigravity、automation、claude、claude-code
- 项目类型：AI/Agent 工程项目
...
![ComposioHQ/awesome-claude-skills 架构拆解图](/img/daily-reports/2026-07-26-github-composiohq-awesome-claude-skills-architecture.svg)
...
![ComposioHQ/awesome-claude-skills 代码调用链图](/img/daily-reports/2026-07-26-github-composiohq-awesome-claude-skills-call-chain.svg)
...
- README 结构：Give your skills real-world actions / Quickstart: Connect Claude to 1000+ Apps / 1. Install the Plugin / 2. Run Setup / 3. Restart & Try It / Contents
- 开篇信息：A comprehensive and curated list of 1000+ production ready and practical Claude Skills and Plugins for enhancing productivity across usecases on not just Claude.ai, Claude Code, but also across coding agents like Codex, Cursor, Gemini CLI, Antigravity and more. Skills tell your agent **how** to work. An MCP Gateway gives it secure access to the tools it needs. Composio [MCP Gateway](https://composio.dev/mcp-gateway) provides a single MCP endpoint for 1,000+ integrations with built-in authentication, team-based acce
...
| `connect-apps-plugin/README.md` | 确认项目承诺、安装方式、核心概念和使用边界。 |
| `document-skills/docx/ooxml/schemas/ISO-IEC29500-4_2016/dml-main.xsd` | 用于定位项目的核心边界和上下游依赖。 |
| `document-skills/docx/ooxml/schemas/ISO-IEC29500-4_2016/vml-main.xsd` | 用于定位项目的核心边界和上下游依赖。 |
| `document-skills/pdf/scripts/check_bounding_boxes_test.py` | 追踪可执行逻辑，确认脚本承担的是采集、转换、执行还是验证。 |
| `document-skills/pptx/ooxml/schemas/ISO-IEC29500-4_2016/dml-main.xsd` | 用于定位项目的核心边界和上下游依赖。 |
| `document-skills/pptx/ooxml/schemas/ISO-IEC29500-4_2016/vml-main.xsd` | 用于定位项目的核心边界和上下游依赖。 |
| `mcp-builder/reference/node_mcp_server.md` | 看工具注册、权限、参数 schema、超时和失败回退。 |
| `mcp-builder/reference/python_mcp_server.md` | 看工具注册、权限、参数 schema、超时和失败回退。 |
| `webapp-testing/scripts/with_server.py` | 追踪可执行逻辑，确认脚本承担的是采集、转换、执行还是验证。 |
