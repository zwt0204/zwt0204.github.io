---
layout: post
title: "GitHub Trending 精读：macro-inc/macro (2026-08-13)"
subtitle: "单个开源项目深度拆解"
date: 2026-08-13
author: "zwt"
...
- 语言：Rust
- Stars：1,998，Forks：234，今日新增：227
- Topics：agent、ai、ai-agents、all-in-one、crm、crm-system
- 官网/演示：[https://macro.com](https://macro.com)
- 项目类型：AI/Agent 工程项目
...
![macro-inc/macro 架构拆解图](/img/daily-reports/2026-08-13-github-macro-inc-macro-architecture.svg)
...
![macro-inc/macro 代码调用链图](/img/daily-reports/2026-08-13-github-macro-inc-macro-call-chain.svg)
...
- README 结构：Why Macro / Features / **Multiple email inboxes** w/ good AI tools, integrated CRM / **Team chat** for focused technical discussions / **Task management** built around chat / @linked markdown docs powered by CRDTs
- 开篇信息：· · ·
...
- `crates/ai_projections/Cargo.toml`
- `crates/ai_tools/Cargo.toml`
- `crates/ai_toolset/Cargo.toml`
- `crates/ai_usage/Cargo.toml`
- `crates/analytics_client/Cargo.toml`
- `crates/anthropic/Cargo.toml`
- `crates/anthropic/README.md`
- `crates/attachment/Cargo.toml`
- `crates/authentication_service_client/Cargo.toml`

### 关键文件怎么读

| 文件/目录 | 阅读重点 |
| --- | --- |
| `crates/activity/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/agent/Cargo.toml` | 看任务状态、计划、执行循环和 observation 如何更新。 |
| `crates/agent_runtime_protocol/Cargo.toml` | 看任务状态、计划、执行循环和 observation 如何更新。 |
| `crates/ai_projections/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/ai_tools/Cargo.toml` | 看工具注册、权限、参数 schema、超时和失败回退。 |
| `crates/ai_toolset/Cargo.toml` | 看工具注册、权限、参数 schema、超时和失败回退。 |
| `crates/ai_usage/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/analytics_client/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/anthropic/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/anthropic/README.md` | 确认项目承诺、安装方式、核心概念和使用边界。 |
| `crates/attachment/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/authentication_service_client/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
