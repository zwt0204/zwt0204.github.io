---
layout: post
title: "GitHub Trending 精读：1jehuang/jcode (2026-07-30)"
subtitle: "单个开源项目深度拆解"
date: 2026-07-30
author: "zwt"
...
- 语言：Rust
- Stars：13,588，Forks：1,490，今日新增：640
- Topics：ai、ai-agent、ai-coding-agent、claude、cli、coding-agent
- 官网/演示：[https://jcode.sh](https://jcode.sh)
- 项目类型：AI/Agent 工程项目
...
![1jehuang/jcode 架构拆解图](/img/daily-reports/2026-07-30-github-1jehuang-jcode-architecture.svg)
...
![1jehuang/jcode 代码调用链图](/img/daily-reports/2026-07-30-github-1jehuang-jcode-call-chain.svg)
...
- README 结构：jcode / Installation / macOS & Linux / Windows 11 (PowerShell 5.1+) / Performance & Resource Efficiency / RAM comparison
- 开篇信息：The most RAM efficient harness <br> The most most intelligent harness curl -fsSL https://jcode.sh/install | bash

值得优先打开的文件或目录：

- `crates/jcode-agent-runtime/Cargo.toml`
...
- `crates/jcode-background-types/Cargo.toml`
- `crates/jcode-base/Cargo.toml`
- `crates/jcode-batch-types/Cargo.toml`
- `crates/jcode-build-meta/Cargo.toml`
- `crates/jcode-build-support/Cargo.toml`
- `crates/jcode-command-risk/Cargo.toml`
...
| 文件/目录 | 阅读重点 |
| --- | --- |
| `crates/jcode-agent-runtime/Cargo.toml` | 看任务状态、计划、执行循环和 observation 如何更新。 |
| `crates/jcode-ambient-types/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-app-core/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-auth-types/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-azure-auth/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-background-types/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-base/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-batch-types/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-build-meta/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-build-support/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-command-risk/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `crates/jcode-compaction-core/Cargo.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
