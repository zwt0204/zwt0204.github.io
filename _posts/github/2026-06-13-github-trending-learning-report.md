---
layout: post
title: "GitHub Trending 精读：addyosmani/agent-skills (2026-06-13)"
subtitle: "单个开源项目深度拆解"
date: 2026-06-13
author: "zwt"
...
- 语言：Shell
- Stars：66,247，Forks：7,147，今日新增：2,656
...
![addyosmani/agent-skills 架构拆解图](/img/daily-reports/2026-06-13-github-addyosmani-agent-skills-architecture.svg)
...
![addyosmani/agent-skills 代码调用链图](/img/daily-reports/2026-06-13-github-addyosmani-agent-skills-call-chain.svg)
...
- README 结构：Agent Skills / Commands / Quick Start / All 24 Skills / Meta - Discover which skill applies / Define - Clarify what to build
...
| `docs/agents.md` | 用于定位项目的核心边界和上下游依赖。 |
...
| `AGENTS.md` | 用于定位项目的核心边界和上下游依赖。 |
| `agents/test-engineer.md` | 看测试覆盖的是数据结构、转换规则、执行脚本还是端到端路径。 |
| `commands/test.toml` | 看配置约束、默认行为、兼容平台和发布/集成方式。 |
| `hooks/session-start-test.sh` | 看测试覆盖的是数据结构、转换规则、执行脚本还是端到端路径。 |
| `hooks/simplify-ignore-test.sh` | 看测试覆盖的是数据结构、转换规则、执行脚本还是端到端路径。 |
| `references/testing-patterns.md` | 看测试覆盖的是数据结构、转换规则、执行脚本还是端到端路径。 |
