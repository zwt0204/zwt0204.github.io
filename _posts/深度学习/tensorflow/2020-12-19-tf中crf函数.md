...
$$
\begin{array}{l}{\text { Transition Score: }} \\ {\text { Transition. Score = }} \\ {t_{S T A R T->B-\text {Person}}+t_{B-\text {Person}->I-\text {Person}}+} \\ {t_{I-\text {Person}->O+t_{0->B-\text {Organization}}+t_{B-\text {Organization}-}}}t_{0}
\\->END\end{array}
$$
- 则可以将损失函数简化为：
$$
